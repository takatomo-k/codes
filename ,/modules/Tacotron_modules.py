...
from modules.Attention_modules import LocalGMMScorerAttention
...
class CBHG(nn.Module):
    ...
    def forward(self, input_):
        ...
        # Bidirectional GRU
        if use_cuda:
            init_gru =torch.zeros(2 * self.num_gru_layers, batch_size, self.hidden_size).cuda()
        else:
            init_gru = torch.zeros(2 * self.num_gru_layers, batch_size, self.hidden_size)
...
class AttentionDecoder(nn.Module):
    ...
    def __init__(self, num_units,num_mels,outputs_per_step):
        ...
        self.num_units = num_units
        #self.att_fn=LocalGMMScorerAttention(num_units,num_units)
        ...
        self.stop = nn.Linear(num_units, outputs_per_step)
    ...
    def forward(self, decoder_input, memory, attn_hidden, gru1_hidden, gru2_hidden):
        ...
        #att_weights,d_t_prime=self.att_fn(d_t,memory)
        #"""
        # Get keys
        ...
        #import pdb; pdb.set_trace()
        att_weights = self.v(F.tanh(keys + d_t_duplicate).view(-1, self.num_units)).view(-1, memory_len, 1)
        ...
        d_t_prime = torch.bmm(att_weights.view([batch_size,1,-1]), memory).squeeze(1)
        #"""
        ...
        #import pdb; pdb.set_trace()
        stop = self.stop(bf_out).view(-1, 1, self.outputs_per_step)
        return output,stop, d_t, gru1_hidden, gru2_hidden,att_weights.unsqueeze(2)
