...
from new_models.Speech2Speech.S2S_network import Speech2Speech
import base
...
class train(base.base):
    def __init__(self):
        ...
        self.model=Speech2Speech(len(self.dataset.src_id2word),len(self.dataset.trg_id2word))
        self.path=os.path.join(self.path,self.model.get_path(),"ver2",self.args.segment)
    ...
    def load(self,):
        if super().__load__():
            if os.path.exists(os.path.join(self.path,"train_loss")):
                with open(os.path.join(self.path,"train_loss")) as f:
                    for i in f.readlines():
                        i=i.strip()
                        self.train_loss.append(float(i))

            if os.path.exists(os.path.join(self.path,"/dev_loss")):
                with open(os.path.join(self.path,"dev_loss")) as f:
                    for i in f.readlines():
                        i=i.strip()
    ...
    def save(self,):
        ...
        show_loss(self.train_loss,self.path+"/train_loss.pdf",self.l1_criterion._get_name())
    ...
    def develop(self):
        ...
        for i, (src_mel,src_txt,trg_txt,trg_mel,stop_targets) in self.pbar:
            with torch.no_grad():
                trans_out,mel_out,stop_outputs,att_score,tts_enc,tts_ref=self.model(src_mel.cuda(),src_txt.cuda(),trg_txt.cuda(),trg_mel.cuda(),self.teacher_forcing_ratio,self.phase)
    ...
    def train(self):
        ...
        for i, (src_mel,src_txt,trg_txt,trg_mel,stop_targets) in self.pbar:
            ...
            if self.phase in {0}:
                ...
                train_mel_loss+=torch.nn.functional.l1_loss(mel_out,tts_ref).item()
                train_l1_loss+=loss.item()
            else:
                mel_loss=self.l1_criterion(mel_out.transpose(1,2),trg_mel.cuda())
            ...
            self.timesteps+=1
            self.pbar.set_description(f'Epoch {self.epoch} ph {self.phase} lr {self.lr:.4f} l1: {train_l1_loss/(i+1):.4f} mel: {train_mel_loss/(i+1):.4f} ')
