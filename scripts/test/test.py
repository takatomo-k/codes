...
class train(base.base):
    def __init__(self):
        self.name="ASR"
        super().__init__()
        self.model=ASR(len(self.dataset.src_id2word))
    ...
    def eval(self,trgs,outs,scores,dump=False,f=None):
        loss=0
        for i,(ref,out,score) in enumerate(zip(trgs,outs,scores)):
            ref_sen=self.dataset.i2w4eval(ref,self.args.src_lang)
            out_sen=self.dataset.i2w4eval(np.argmax(out,axis=-1),self.args.src_lang)
            loss+=wer(out_sen,ref_sen)
            if f is not None:
                f.write(out_sen+"|"+ref_sen+"\n")
            if dump:
                out=self.dataset.i2w(np.argmax(out,axis=-1),self.args.src_lang)
                show_attention(score,ref_sen,os.path.join(self.path,self.state,"Attention",str(self.epoch)),label_y=out.split())
        return loss/(i+1)

    def develop(self):
        super().__develop__()
        loss=0
        for i, (mel,txt) in self.pbar:
            with torch.no_grad():
                out_txt,att_score=self.model(mel.cuda())
                loss+=self.eval(txt.numpy(),out_txt.cpu().numpy(),att_score.cpu().numpy(),dump=i<15)
            self.pbar.set_description(f'Develop WER: {loss/(i+1):.4f}')
        self.dev_loss.append(loss/(len(self.dataloader)))

    def test(self):
        super().__test__()
        loss=0
        with open(os.path.join(self.path,"TestWER"),"w") as f:
            for i, (mel,txt) in self.pbar:
                with torch.no_grad():
                    out_txt,att_score=self.model(mel.cuda())
                loss+=self.eval(txt.numpy(),out_txt.cpu().numpy(),att_score.cpu().numpy(),dump=i<15,f=f)
                self.pbar.set_description(f'Test WER: {loss/(i+1):.4f}')
    ...
    def train(self):
        ...
        for i, (mel,txt) in self.pbar:
            ...
            if len(self.dataloader)-i<10:
                train_wer+=self.eval(txt.numpy(),out_txt.detach().cpu().numpy(),att_score.detach().cpu().numpy(),dump=len(self.dataloader)-i==1)
                self.pbar.set_description(f'Epoch {self.epoch} tr {self.teacher_forcing_ratio:.2f} lr {self.lr:.4f} loss {train_loss/(i+1):.3f} WER: {train_wer/(10-len(self.dataloader)+i):.1f}')
            else:
